...
;;;;; MELBIS-V1
;;;;; To cite:
;;;;; Perez, L., Dragicevic, S., & Gaudreau, J. (2019).
...
;;;;; PLOS ONE, 14(7), e0219188. https://doi.org/10.1371/journal.pone.0219188
...
to segregate ;; asks immigrants to move to one of their suitable patches
  ...
  ask Immigrants [ifelse count Choice5 with [not any? turtles-here] > 0 [move-to one-of Choice5 with[not any? turtles-here]]
    [ifelse count Choice5 with [count turtles-here <= 2] > 0 [move-to one-of Choice5 with [count turtles-here <= 2]]
...
;  ask Immigrants ; "settled" variable is in V2, not V1
;  [
...
409
666
INSTRUCTIONS\n\n1) Adjust experiment parameters \"desired_neighbors\" and \"nbBus\" sliders in the top left.\n\n2) Click Setup. Setup is complete when the button returns to its original colour.\n\n2) Click go. The model run is over once the go button returns to its original colour Afterwards you may click Export_To_Shapefile to export data for processing in a GIS.
14
0.0
1
