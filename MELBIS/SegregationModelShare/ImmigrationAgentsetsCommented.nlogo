...
to Export_To_Shapefile
  gis:set-world-envelope-ds gis:envelope-of mtlDs
...
to compute_neighborhoods
  ...
  print "Computing bus count"
  ask patches with [is_mtl = 1] [set buses count patches in-radius 8 with [bus = 1]]
  print "Computing metro count"
  ask patches with [is_mtl = 1] [ set metros count patches in-radius 8 with [metro = 1]]
  print "computing school count"
  ask patches with [is_mtl = 1] [ set schools count patches in-radius 16 with [school = 1]]
  print "computing park count"
  ask patches with [is_mtl = 1] [ set parks count patches in-radius 16 with [park = 1]]
  print "computing commerce count"
  ask patches with [is_mtl = 1] [ set commerces count patches in-radius 32 with [commerce = 1]]
...
ifelse count patches with [not any? Residents-here and africanMean > desired_neighbors and englishMean > 50] > 0
[ask immigrants with [ethnicity = "african" and language = "English"]
[set choice1 patches with [not any? Residents-here and africanMean > desired_neighbors and englishMean > 50]]] [ask immigrants with [ethnicity = "african" and language = "English"][set Choice1  patches with [not any? Residents-here and africanMean > 10 and englishMean > 50]]]
...
ifelse count patches with [not any? Residents-here and americanMean > desired_neighbors and englishMean > 50] > 0
[ask immigrants with [ethnicity = "american" and language = "English"]
[set choice1 patches with [not any? Residents-here and americanMean > desired_neighbors and englishMean > 50]]] [ask immigrants with [ethnicity = "american" and language = "English"] [set Choice1  patches with [not any? Residents-here and africanMean > 10 and englishMean > 50]]]
...
[ask immigrants with [ethnicity = "asian" and language = "English"]
[set choice1 patches with [not any? Residents-here and asianMean > desired_neighbors and englishMean > 50]]] [ask immigrants with [ethnicity = "asian" and language = "English"] [set Choice1  patches with [not any? Residents-here and africanMean > 10 and englishMean > 50]]]
...
ifelse count patches with [not any? Residents-here and europeanMean > desired_neighbors and englishMean > 50] > 0
[ask immigrants with [ethnicity = "european" and language = "English"]
[set choice1 patches with [not any? Residents-here and europeanMean > desired_neighbors and englishMean > 50]]] [ask immigrants with [ethnicity = "european" and language = "English"] [set Choice1  patches with [not any? Residents-here and africanMean > 10 and englishMean > 50]]]
...
to select_choice2
;;;;;;;;;;;;;;;; Choice 2 is based on income -JLK
  print "Subsetting patches from choice1 in regards to the income"
  ask Immigrants with [count Choice1 > 1] [
    ifelse (count Choice1 with [medincome > [income] of myself - 10000 and medincome < [income] of myself + 5000] > 0) [set Choice2 Choice1 with [medincome > [income] of myself - 10000 and medincome < [income] of myself + 5000]][set Choice2 Choice1]]
  print "Finished"
  end
...
;;;;;;;;;;;;;;;; Choice 3 is based on access to schools -JLK
  ...
  ask Immigrants with [nbChildren > 0] [ifelse count Choice2 with [schools > 0] > 0 [set Choice3 Choice2 with [schools > 0]][set Choice3 Choice2]]
...
;;;;;;;;;;;;;;;; Choice 4 is based on neighborhood quality (parks and commerce) -JLK
  ...
  ask Immigrants with [typeImmigrant = "Business"] [if count Choice3 with [parks > 0 and commerces > 0] > 0 [set Choice4 Choice3 with [parks > 0 and commerces > 0] set defined? true]]
  ask n-of (0.5 * count Immigrants with [typeImmigrant = "Skilled Worker"]) Immigrants with [typeImmigrant = "Skilled Worker"] [if count Choice3 with [parks > 0 and commerces > 0] > 0 [set Choice4 Choice3 with [parks > 0 and commerces > 0] set defined? true]]
  ask Immigrants with [defined? != true] [set Choice4 Choice3]
end

to select_choice5
;;;;;;;;;;;;;;;; Choice 5 is based on access to public transit -JLK
  ask Immigrants [ifelse count Choice4 with [(buses > nbBus) or (metros > 0)] > 0 [set Choice5 Choice4 with [buses > nbBus or metros > 0]] [set Choice5 Choice4]]
end
to importGis
  print "Importing GIS Layers..."
  ask patches [set pcolor white]
  set mtlDs gis:load-dataset "mtl.asc"
  gis:set-world-envelope-ds gis:envelope-of mtlDs
  gis:apply-raster mtlDs is_mtl
  ask patches with [is_mtl = 1] [set pcolor gray + 1]

;;;;;;;;;;;;;;;;;;;;;;;; Population datasets represent the percentage of population with the given variable present in a cell -JLK
...
;;;;;;;;;;;;;;;;;;;;;;;; The following variables are represented as binary rasters -JLK
  ...
  set busDs gis:load-dataset "2011_bus.asc"
  ...
  set metroDs gis:load-dataset "2011_metro.asc"
...
;;;;;;;;;;;;;;;;;;;;;;;; Residents do not move, Immigrants will move -JLK
  ...
  ask n-of 6667 patches with [is_mtl = 1] [sprout-Residents 1 [set color blue set size 1 ]]
  ask n-of 3333 patches with [is_mtl = 1 and not any? turtles-here] [sprout-Immigrants 1 [set size 1 set satisfied? false set step 1]]
end

to giveValueToImmigrants
;;;;;;;;;;;;;;;;;;;;;;;; This procedure gives a demographic profile to each immigrant based on statistics of immigration in Quebec in 2010 -JLK
  ask n-of (0.368 * 3333) Immigrants [set color black set nbChildren random 4 set ethnicity "african" set defined? true]
  ask n-of (0.212 * 3333) Immigrants with [defined? != true] [set color pink set nbChildren random 4 set ethnicity "american" set defined? true]
  ask n-of (0.254 * 3333) Immigrants with [defined? != true] [set color yellow set nbChildren random 4 set ethnicity "asian" set defined? true]
  ask Immigrants with [defined? != true] [set color white set nbChildren random 4 set ethnicity "european"]
  ask Immigrants [set defined? false]
  ask n-of (0.78 * 3333) Immigrants [set typeImmigrant "Skilled Worker" set defined? true]
  ask n-of (0.09 * 3333) Immigrants with [defined? != true] [set typeImmigrant "Business" set defined? true]
  ask Immigrants with [defined? != true] [set typeImmigrant "Refugee"]
  ask Immigrants[set defined? false]
  ask n-of (0.7 * 3333) Immigrants [set age 18 + random 18 set defined? true]
  ask Immigrants with [defined? != true] [set age 36 + random 11]
  ask Immigrants [set defined? false]
  ask Immigrants[
    ifelse (nbChildren = 1) [set income 11931]
    [ifelse (nbChildren = 2) [set income 14853]
      [ifelse (nbChildren = 3) [set income 18260]
        [ifelse (nbChildren = 4) [set income 22170]
          [ifelse (nbChildren = 5) [set income 25145]
            [ifelse (nbChildren = 6) [set income 28359]
              [set income 31574]]]]]]]
  ask Immigrants [ifelse random-float 1 > 0.5 [set education "Superior"] [set education "None"]]
  ask n-of (0.651 * 3333) Immigrants [set language "French" set defined? true]
  ask n-of (0.049 * 3333) Immigrants with [defined? != true] [set language "English" set defined? true]
  ask Immigrants with [defined? != true] [set language "Both" set defined? true]
  ask Immigrants [set defined? false]

end

to autoTest
;;;;;;;;;;;;;;;;;;;;;;;; I added this procedure to more easily run repeat tests -JLK
  setup
  go
;;;;;;;;;;;;;;;;;;;;;;;; This part is the code found in the "Go To Choice 5" button -JLK
  ask Immigrants [ifelse count Choice5 with [not any? turtles-here] > 0 [move-to one-of Choice5 with
[not any? turtles-here]]
[ifelse count Choice5 with [count turtles-here < 2] > 0
[move-to one-of Choice5 with [count turtles-here < 2]]
...
376
Go to Choice 3
ask Immigrants [ifelse count Choice3 with [not any? turtles-here] > 0 [move-to one-of Choice3 with\n[not any? turtles-here]]\n[ifelse count Choice3 with [count turtles-here < 2] > 0\n[move-to one-of Choice3 with [count turtles-here < 2]]\n[move-to one-of Choice3]]]
NIL
1
T
OBSERVER
...
384
565
Dans l'ordre:\n\nSetup\nselect_choice1\nselect_choice2\nselect_choice3\nselect_choice4\nselect_choice5\nGo To Choice 5 (for scenario 1)\nGo to Choice 3 (for scenario 2)\nExport to Shapefile (will export the points)
14
0.0
1
...
@#$#@#$#@
<experiments>
  <experiment name="experiment" repetitions="18" runMetricsEveryStep="false">
    <go>autoTest</go>
